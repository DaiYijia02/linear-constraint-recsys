...
if __name__ == "__main__":
    ...
    filtered_matrix = small_matrix.loc[small_matrix['video_id'].isin(
        filtered_videos)].set_index(["user_id", "video_id"])
    relevance = (filtered_matrix["watch_ratio"] /
                 2).clip(upper=1).rename("relevance")
...
#    np.save(EXP_DIR / "train.npy", D)
#    np.save(EXP_DIR / "dev.npy", V)
#    np.save(EXP_DIR / "test.npy", R)
    np.save("train.npy", D)
    np.save("dev.npy", V)
    np.save("test.npy", R)
