...
class Simulator(object):
    def __init__(self, controller, m, N, MAX_UTIL, delta, C, test=False, tqdm_disable=True):
    ...
    def get_metrics(self, delta, tau=None):
        ...
        unsatisfaction[unsatisfaction < 1e-7] = 0.

        weighted = np.sum(self.utility.copy()) - np.clip(delta - self.state, 0, None).copy().sum()
        weighted_objective = np.sum(self.utility.copy()) - self.C @ np.clip(delta - self.state, 0, None).copy()
        return {
            ...
            "Sacrifice Ratio": (self.MAX_UTIL - np.mean(self.utility.copy())) / self.MAX_UTIL,
            ...
            "C * Unsatisfaction Sum": self.C @ np.clip(delta - self.state, 0, None).copy(),
            "delta": delta.copy(),
